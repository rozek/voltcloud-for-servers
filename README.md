...
* **`async function actOnBehalfOfDeveloper (EMailAddress:string, Password:string):Promise<void>`**<br>uses the given `EMailAddress` and `Password` to request an "access token" from VoltCloud, which is then used to authenticate any non-public VoltCLoud request. Note: `EMailAddress` and `Password` are stored in memory in order to automatically refresh the token upon expiry<br>&nbsp;<br>
